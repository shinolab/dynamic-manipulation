...
namespace dynaman {
	...
	odcs::odcs(Tracker& tracker) :m_tracker(tracker), flagRunning(false) {};
	...
	void odcs::SetSensor(Tracker& new_tracker) {
	...
	void odcs::ControlLoop(std::vector<FloatingObjectPtr>& objPtrs, int loopPeriod = 33)
	{
		for (auto itr = objPtrs.begin(); itr != objPtrs.end(); itr++)
		{
			...
			bool observed = m_tracker.observe(observationTime, posObserved, *itr);
