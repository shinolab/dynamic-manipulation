...
NPRESULT NPClient_Init( LPTSTR pszDLLPath )
{
	...
	std::cout << "Initializing NPClient..." << std::endl;

	if (pszDLLPath == NULL) {
		std::cerr << "pszDLLPath is NULL." << std::endl;
		return NP_ERR_DLL_NOT_FOUND;
	}

	_tcscpy_s(szFullPath, pszDLLPath);

	if (lstrlen(szFullPath) > 0)        
	{
		_tcscat_s(szFullPath, _T("\\"));
	}
	
#if defined(_M_X64) || defined(__amd64__) 
	_tcscat_s(szFullPath, _T("NPClient64.dll"));
#else
	_tcscat_s(szFullPath, _T("NPClient.dll"));
#endif 
	
	ghNPClientDLL = ::LoadLibrary( szFullPath );
	std::cout << "path to dll: " << std::endl << szFullPath << std::endl;
	if( NULL != ghNPClientDLL )
	{
		...
		std::cout
			<< "[verify signature]" << std::endl
			<< "DLL: " << verifySignature.DllSignature << std::endl
			<< "APP:" << verifySignature.AppSignature << std::endl
			<< "[pSignature]" << std::endl
			<< "DLL: " << pSignature.DllSignature << std::endl
			<< "APP: " << pSignature.AppSignature << std::endl;
		...
		if( vresult == NP_OK )
		{
			...
			{	
				...
				if((strcmp(verifySignature.DllSignature, pSignature.DllSignature) != 0)){
					std::cerr << "Error found in dllSignature" << std::endl;
				}
				if ((strcmp(verifySignature.AppSignature, pSignature.AppSignature) != 0)) {
					std::cerr << "Error found in AppSignature" << std::endl;
				}
	...
	{
		std::cerr << "ghNPCliendDLL is NULL." << std::endl;
		result = NP_ERR_DLL_NOT_FOUND;
	}
