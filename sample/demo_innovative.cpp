...
int main() {
	...
	FloatingObjectPtr objPtr = FloatingObject::Create(posRight, -0.0001f, 79.f);
	...
	objPtr->SetTrajectory(std::shared_ptr<Trajectory>(new TrajectoryBangBang(2.0f, timeGetTime() / 1000.f, posRight, posLeft)));
	...
	objPtr->SetTrajectory(std::shared_ptr<Trajectory>(new TrajectoryBangBang(2.0f, timeGetTime() / 1000.f, posLeft, posRight)));
	std::this_thread::sleep_until(std::chrono::system_clock::now() + std::chrono::seconds(8)); // wait until the floating object is stabilized.
