...
int main() {
	...
	auto objPtr1 = FloatingObject::Create(Eigen::Vector3f(-423.86f, 596.04f + 100, 1431.f));
	auto objPtr2 = FloatingObject::Create(Eigen::Vector3f(0, 0, 200));
	...
	auto duties = dynaman.ocsPtr->FindDutyQPMulti(forcesToApply, positions);
	std::cout << duties.size() << std::endl;
	std::cout << duties << std::endl;
	dynaman.Close();
	return 0;
}